...
impl Cable {
    ...
    /// `core_ids` prints the internal ids of the cores generated by `insert_cores`.
    ///
    /// This data is useful when assigning connection data in the datafiles and can be printed with
    ...
    pub fn core_ids(
        ...
        for (id, ref_core) in reference_cores {
            ...
            match core_type.cores.len().cmp(&1) {
                Ordering::Greater => {
                    // recursive call to handle inner cables
                    let sub_core_ids: Vec<String> = self.core_ids(&core_type.cores, library, Some(core_id_stripped))?;
